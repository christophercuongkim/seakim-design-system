...
| `RosterList.jsx` | roster | Bench's column definition over the shared `Table`. The species swap comes from `Table` itself. |
...
| `LeagueScreen.jsx` | League | Standings over the shared `Table`; the two widest columns drop at md, your own row is the one accent |
...
- **List at sm, table from md up.** Not a scrolling table — a genuinely different row
  layout, because a 6-column table cannot be read on a phone. Both layouts now come from
  the shared `Table`, so there is one implementation rather than two.
