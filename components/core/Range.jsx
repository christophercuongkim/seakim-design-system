 ...
 *  promote one with `accent`. See decision 0017. */
export function Range({ low, mid, high, domain, size = 'md', accent = false, label, style, ...rest }) {
  ...
  return (
    <div
      ...
      {...rest}
    >
      <div style={{ position: 'absolute', inset: 0, background: 'var(--surface-inset)', border: '1px solid var(--border-subtle)' }} />
      <div style={{ position: 'absolute', top: 0, bottom: 0, left: `${bandL}%`, width: `${bandW}%`, background: accent ? 'var(--fill-accent)' : 'var(--text-tertiary)' }} />
      <div style={{ position: 'absolute', top: -1, bottom: -1, left: `${pct(mid, d0, d1)}%`, width: s.marker, transform: 'translateX(-50%)', background: 'var(--text-primary)' }} />
    </div>
  );
}
