...
const chartSeries = Object.entries(src.chart ?? {})
  // Numeric keys only: `seq` lives alongside them but is a ramp, not a series, and
  // has no l/c/h of its own — reading it as one crashes the build.
...
/** chart.seq — the sequential ramp. Per-theme, validated separately (0015). */
const chartSeq = src.chart?.seq
  ? {
      hue: src.chart.seq.hue.$value,
      light: ['1', '2', '3', '4'].map(k => ({ step: k, l: src.chart.seq.light[k].l.$value, c: src.chart.seq.light[k].c.$value })),
      dark: ['1', '2', '3', '4'].map(k => ({ step: k, l: src.chart.seq.dark[k].l.$value, c: src.chart.seq.dark[k].c.$value })),
      inkFlip: { light: 4, dark: 3 },
    }
  : null;
...
function emitCss() {
  ...
  L.push('');
  if (chartSeq) {
    L.push('  /* Sequential ramp for magnitude — heatmaps, rank grids, choropleths. Fixed hue,');
    L.push('     product-independent, per decision 0015. Dark ascends into lightness from just');
    L.push('     above --surface-card. Label ink flips to --text-inverse at step 3 in dark. */');
    for (const s of chartSeq.dark) {
      L.push(`  --chart-seq-${s.step}: oklch(${s.l.toFixed(2)} ${s.c.toFixed(3)} ${chartSeq.hue});`);
    }
    L.push(`  --chart-seq-ink-flip: ${chartSeq.inkFlip.dark};`);
    L.push('');
  }
...
function emitCssLight() {
  ...
  if (chartSeq) {
    L.push('');
    L.push('  /* Sequential ramp, light. Ascends into darkness from just below');
    L.push('     --surface-card; ink flips to --text-inverse at step 4. Validated per');
    L.push('     theme rather than flipped, per decision 0015. */');
    for (const s of chartSeq.light) {
      L.push(`  --chart-seq-${s.step}: oklch(${s.l.toFixed(2)} ${s.c.toFixed(3)} ${chartSeq.hue});`);
    }
    L.push(`  --chart-seq-ink-flip: ${chartSeq.inkFlip.light};`);
    L.push('');
  }
...
function emitDart() {
  ...
  L.push('');
  if (chartSeq) {
    L.push('/// Sequential ramp for magnitude — heatmaps, rank grids, choropleths.');
    L.push('///');
    L.push('/// Per-theme rather than one ramp inverted: each direction is validated on its');
    L.push('/// own surface, per decision 0015.');
    L.push('class SkChartSeq {');
    L.push('  const SkChartSeq({');
    L.push('    required this.s1,');
    L.push('    required this.s2,');
    L.push('    required this.s3,');
    L.push('    required this.s4,');
    L.push('    required this.inkFlip,');
    L.push('  });');
    L.push('');
    for (const f of ['s1', 's2', 's3', 's4']) L.push(`  final Color ${f};`);
    L.push('');
    ...
    L.push('        _ => s4,');
    L.push('      };');
    for (const [theme, steps, note] of [
      ['light', chartSeq.light, 'Ascends into darkness.'],
      ['dark', chartSeq.dark, 'Ascends into lightness.'],
    ]) {
      L.push('');
      L.push(`  /// ${note}`);
      L.push(`  static const SkChartSeq ${theme} = SkChartSeq(`);
      for (const st of steps) {
        L.push(`    s${st.step}: Color(${argb(st.l, st.c, chartSeq.hue)}),`);
      }
      L.push(`    inkFlip: ${chartSeq.inkFlip[theme]},`);
      L.push('  );');
    }
    L.push('}');
    L.push('');
...
function emitTs() {
  ...
  if (chartSeq) {
    L.push('');
    L.push('/** Sequential ramp for magnitude. Validated per theme, never flipped. */');
    L.push('export const skChartSeq = {');
    for (const [theme, steps] of [['light', chartSeq.light], ['dark', chartSeq.dark]]) {
      const cells = steps.map(st => `'${st.step}': '${hex(st.l, st.c, chartSeq.hue)}'`).join(', ');
      L.push(`  ${theme}: { ${cells}, inkFlip: ${chartSeq.inkFlip[theme]} },`);
    }
    L.push('} as const;');
  }
  return L.join('\n');
